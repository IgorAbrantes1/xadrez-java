...
public class ChessMatch {
    ...
    private Piece makeMove(Position source, Position target) {
        ...
        this.castlingMakeMove(source, target, p);

        // #special move: en passant
        capturedPiece = this.enPassantMakeMove(source, target, p, capturedPiece);

        return capturedPiece;
    }

    private Piece enPassantMakeMove(Position source, Position target, ChessPiece p, Piece capturedPiece) {
        // #special move: en passant
    ...
    private void castlingMakeMove(Position source, Position target, ChessPiece p) {
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece) {
        ...
        // #special move: castling
        this.castlingUndoMove(source, target, p);

        // #special move: en passant
        this.enPassantUndoMove(source, target, capturedPiece, p);
    }

    private void enPassantUndoMove(Position source, Position target, Piece capturedPiece, ChessPiece p) {
        // #special move: en passant
        if (p instanceof Pawn) {
    ...
    private void castlingUndoMove(Position source, Position target, ChessPiece p) {
        // #special move: castling king's side rook
        if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
            Position sourceRook = new Position(source.getRow(), source.getColumn() + 3);
            Position targetRook = new Position(source.getRow(), source.getColumn() + 1);
